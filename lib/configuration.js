(function() {
  var Configuration, Logger, async, exec, fs, getFilenamesForHost, libraryPath, mkdirp, path, sourceScriptEnv;
  ...
  module.exports = Configuration = (function() {
    ...
    function Configuration(options) {
      ...
      domain = (_ref13 = (_ref14 = options.domain) != null ? _ref14 : process.env['POW_DOMAIN']) != null ? _ref13 : "test";
    ...
    Configuration.prototype.firewallLaunchAgentSource = function() {
      return require("./cx.pow.firewall.plist")(this);
    };
    Configuration.prototype.daemonLaunchAgentSource = function() {
      return require("./cx.pow.powd.plist")(this);
    };
    Configuration.prototype.resolverSource = function() {
      return "nameserver 127.0.0.1\nport " + this.dnsPort + "\n";
    };
    Configuration.prototype.install = function(callback) {
      var writeDaemonLaunchAgent, writeFirewallLaunchAgent, writeResolvers;
      if (process.env['USER'] !== 'root') {
        return callback(new Error("Installation requires root"));
      } else {
        writeFirewallLaunchAgent = __bind(function(callback) {
          var filename;
          filename = "/Library/LaunchDaemons/cx.pow.firewall.plist";
          return fs.writeFile(filename, this.firewallLaunchAgentSource(), callback);
        }, this);
        writeDaemonLaunchAgent = __bind(function(callback) {
          var filename;
          filename = "" + process.env['HOME'] + "/Library/LaunchAgents/cx.pow.powd.plist";
          return fs.writeFile(filename, this.daemonLaunchAgentSource(), function(err) {
            if (err) {
              return callback(err);
            } else {
              return exec("chown " + process.env['SUDO_USER'] + ":staff " + filename, callback);
            }
          });
        }, this);
        writeResolvers = __bind(function(callback) {
          return async.map(this.domains, __bind(function(domain, callback) {
            return fs.writeFile("/etc/resolver/" + domain, this.resolverSource(), callback);
          }, this), callback);
        }, this);
        return async.parallel([writeFirewallLaunchAgent, writeDaemonLaunchAgent, writeResolvers]);
      }
    };
