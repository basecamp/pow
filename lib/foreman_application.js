(function() {
  var ForemanApplication, HttpProxy, async, basename, bufferLines, exec, exists, fs, join, net, pause, sourceScriptEnv, spawn, _ref, _ref2, _ref3;
  ...
  module.exports = ForemanApplication = (function() {
    ...
    ForemanApplication.prototype.captureForemanPorts = function(line) {
      var count, err, name, pidMatch, portInUseMatch, portMatch, process, processCount, readyCallback, _base, _base2, _i, _j, _len, _len2, _name, _name2, _ref4, _ref5, _ref6;
      ...
      if (pidMatch) {
        (_base = this.webProcesses)[_name = pidMatch[1]] || (_base[_name] = {});
        this.webProcesses[pidMatch[1]].pid = pidMatch[2];
      }
      portMatch = /.* (web\.[0-9]+).*Server running at http\:[^:]*:([0-9]+)/(line);
      if (portMatch) {
        (_base2 = this.webProcesses)[_name2 = portMatch[1]] || (_base2[_name2] = {});
        this.webProcesses[portMatch[1]].port = portMatch[2];
      }
      count = 0;
      processCount = 0;
      _ref5 = this.webProcesses;
      for (name in _ref5) {
        process = _ref5[name];
        processCount++;
        if ((process != null) && (process.pid != null) && (process.port != null)) {
          count++;
        }
      }
      this.webProcessCount = processCount;
      if (count > 0 && count === this.webProcessCount) {
        this.changeState("ready");
        _ref6 = this.readyCallbacks;
        for (_j = 0, _len2 = _ref6.length; _j < _len2; _j++) {
          readyCallback = _ref6[_j];
          readyCallback();
        }
