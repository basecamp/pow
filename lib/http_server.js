(function() {
  var ForemanApplication, HttpProxy, HttpServer, RackApplication, connect, dirname, exists, fs, join, pause, sys, url, version, _ref;
  ...
  module.exports = HttpServer = (function() {
    ...
    function HttpServer(configuration) {
      ...
      this.findForemanApplication = __bind(this.findForemanApplication, this);
      ...
      HttpServer.__super__.constructor.call(this, [o(this.logRequest), o(this.annotateRequest), o(this.handlePowRequest), o(this.findHostConfiguration), o(this.handleStaticRequest), o(this.findRackApplication), o(this.findForemanApplication), o(this.handleProxyRequest), o(this.handleApplicationRequest), x(this.handleErrorStartingApplication), o(this.handleFaviconRequest), o(this.handleApplicationNotFound), o(this.handleWelcomeRequest), o(this.handleRailsAppWithoutRackupFile), o(this.handleLocationNotFound)]);
      this.staticHandlers = {};
      this.rackApplications = {};
      this.foremanApplications = {};
      this.requestCount = 0;
      this.accessLog = this.configuration.getLogger("access");
      this.on("close", __bind(function() {
        var application, root, _ref2, _ref3, _results;
        _ref2 = this.rackApplications;
        for (root in _ref2) {
          application = _ref2[root];
          application.quit();
        }
        _ref3 = this.foremanApplications;
        _results = [];
        for (root in _ref3) {
          application = _ref3[root];
    ...
    HttpServer.prototype.findHostConfiguration = function(req, res, next) {
      var resume;
      resume = pause(req);
      return this.configuration.findHostConfiguration(req.pow.host, __bind(function(err, domain, config) {
        if (config) {
          if (config.root) {
    ...
    HttpServer.prototype.findForemanApplication = function(req, res, next) {
      var root;
      if (!(root = req.pow.root)) {
        return next();
      }
      return exists(join(root, "Procfile"), __bind(function(procfileExists) {
        var application, _base, _ref2;
        if (procfileExists) {
          req.pow.application = (_ref2 = (_base = this.foremanApplications)[root]) != null ? _ref2 : _base[root] = new ForemanApplication(this.configuration, root);
        } else if (application = this.foremanApplications[root]) {
          delete this.foremanApplications[root];
          application.quit();
        }
        return next();
      }, this));
