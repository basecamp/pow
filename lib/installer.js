(function() {
  ...
  InstallerFile = (function() {
    ...
    InstallerFile.prototype.setMode = function(callback) {
      if (this.mode) {
        return fs.chmod(this.path, mode, callback);
      } else {
        return callback(false);
      }
    };
    InstallerFile.prototype.install = function(callback) {
      return async.series([this.vivifyPath, this.writeFile, this.setMode, this.setOwnership], callback);
    };
  ...
  module.exports = Installer = (function() {
    ...
    Installer.getLocalInstaller = function(configuration) {
      this.configuration = configuration;
      return new Installer([new InstallerFile("" + process.env.HOME + "/Library/LaunchAgents/cx.pow.powd.plist", daemonSource(this.configuration), null, 0644)]);
    };
