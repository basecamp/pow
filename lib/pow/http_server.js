(function() {
  var HttpServer, connect, escapeHTML, exec, fs, getHost, join, nack, o, pause, sourceScriptEnv, sys, x;
  ...
  sourceScriptEnv = function(script, callback) {
    var dumpEnvScript;
    dumpEnvScript = "puts \"{\"+ENV.map{|k,v|k.inspect+\": \"+v.inspect}.join(\", \")+\"}\"";
    return exec("source " + script + "; ruby -e '" + dumpEnvScript + "'", function(err, stdout) {
      if (err) {
        return callback(err);
      }
      try {
  ...
  module.exports = HttpServer = (function() {
    ...
    HttpServer.prototype.getHandlerForRoot = function(root, callback) {
      ...
      return this.getEnvForRoot(root, __bind(function(err, env) {
        var handler, _base;
        handler = (_base = this.handlers)[root] || (_base[root] = {
          root: root,
