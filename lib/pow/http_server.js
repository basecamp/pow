(function() {
  var HttpServer, connect, escapeHTML, exec, fs, getHost, join, nack, o, pause, sys, x;
  ...
  module.exports = HttpServer = (function() {
    ...
    HttpServer.prototype.getHandlerForRoot = function(root, callback) {
      ...
      return this.getEnvForRoot(root, __bind(function(err, env) {
        var _base;
        (_base = this.handlers)[root] || (_base[root] = {
          root: root,
    ...
    HttpServer.prototype.getEnvForRoot = function(root, callback) {
      ...
      return fs.stat(path, function(err) {
        if (err) {
          ...
          return exec("source " + path + "; env", function(err, stdout) {
            var env, line, m, _i, _len, _ref;
            if (err) {
              return callback(err);
            }
            env = {};
