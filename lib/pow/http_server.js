(function() {
  ...
  module.exports = HttpServer = (function() {
    ...
    function HttpServer(configuration) {
      ...
      HttpServer.__super__.constructor.call(this, [o(this.logRequest), o(this.handleStaticRequest), o(this.handleRackRequest), x(this.handleApplicationException), o(this.handleNonexistentDomain)]);
      this.staticHandlers = {};
      this.rackHandlers = {};
      this.accessLog = this.configuration.getLogger("access");
      this.on("close", this.closeApplications);
    }
    HttpServer.prototype.getStaticHandlerForHost = function(host, callback) {
      return this.configuration.findApplicationRootForHost(host, __bind(function(err, root) {
        if (err) {
          return callback(err);
        }
        return this.getStaticHandlerForRoot(root, callback);
      }, this));
    };
    HttpServer.prototype.getRackHandlerForHost = function(host, callback) {
      return this.configuration.findApplicationRootForHost(host, __bind(function(err, root) {
        if (err) {
          return callback(err);
        }
        return this.getRackHandlerForRoot(root, callback);
      }, this));
    };
    HttpServer.prototype.getStaticHandlerForRoot = function(root, callback) {
      var handler;
      if (!root) {
        return callback();
      } else if (handler = this.staticHandlers[root]) {
        return callback(null, handler);
      } else {
        handler = this.staticHandlers[root] = connect.static(join(root, "public"));
        return callback(null, handler);
      }
    };
    HttpServer.prototype.getRackHandlerForRoot = function(root, callback) {
      var handler;
      if (!root) {
        return callback();
      } else if (handler = this.rackHandlers[root]) {
        return callback(null, handler);
      } else {
        return this.rackHandlers[root] = new RackHandler(this.configuration, root, callback);
      }
    };
    ...
    HttpServer.prototype.handleStaticRequest = function(req, res, next) {
      ...
      host = getHost(req);
      resume = pause(req);
      return this.getStaticHandlerForHost(host, __bind(function(err, handler) {
        if (handler) {
          if (err) {
            next(err);
            return resume();
    ...
    HttpServer.prototype.handleRackRequest = function(req, res, next) {
      ...
      return this.getRackHandlerForHost(host, __bind(function(err, handler) {
        ...
        if (handler) {
          if (err) {
            ...
            return handler.handle(req, res, next, resume);
          }
        } else {
          this.handleNonexistentDomain(req, res, next);
          return resume();
