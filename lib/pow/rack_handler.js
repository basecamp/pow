(function() {
  var LineBuffer, RackHandler, basename, bufferLines, dirname, fs, getEnvForRoot, join, nack, sourceScriptEnv, _ref;
    ...
    function RackHandler(configuration, root, callback) {
      ...
      installLogHandlers = __bind(function() {
        ...
        bufferLines(this.app.pool.stderr, __bind(function(line) {
          return this.logger.warn(line);
        }, this));
        _ref = ["ready", "spawn", "busy", "exit"];
        _results = [];
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          event = _ref[_i];
          _results.push(__bind(function(event) {
            return this.app.pool.on("worker:" + event, __bind(function(process) {
              return this.logger.debug("nack worker " + process.child.pid + " " + event);
            }, this));
          }, this)(event));
        }
        return _results;
