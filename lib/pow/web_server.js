(function() {
  var WebServer, connect, fs, idle, nack, path, sys;
  ...
  exports.WebServer = WebServer = (function() {
    ...
    WebServer.prototype.createApplicationPool = function(config) {
      var app, root;
    ...
    WebServer.prototype.applicationForConfig = function(config) {
      var _base, _ref;
      if (config) {
        return (_ref = (_base = this.applications)[config]) != null ? _ref : _base[config] = this.createApplicationPool(config);
      }
    };
    WebServer.prototype.onRequest = function(req, res, next) {
      var host, pause;
